...
app.use(express.static('uploads/'));            //local para armazenar uploads na api
...
app.post("/armazenar_coleta", jsonParser, async function (req, res) {
  ...
  const endereco_imagem = salvarImagem(req.body.imagem);
  ...
  connection.query(
    ...
    function (err, result) {
      if (err) throw err;
        ...
        inserir_endereco(result.insertId, endereco, endereco_imagem);
   ...
   */
  function inserir_endereco(id_coleta, endereco, endereco_imagem) {
    endereco.rua = endereco.rua === undefined ? null : `${endereco.rua}`;
    endereco.numero =
      endereco.numero === undefined ? null : `${endereco.numero}`;
    endereco.complemento =
      endereco.complemento === "" ? null : `"${endereco.complemento}"`;
    endereco.bairro =
      endereco.bairro === undefined ? null : `${endereco.bairro}`;
    endereco.cidade =
      endereco.cidade === undefined ? null : `${endereco.cidade}`;
    ...
    req(url, function (err, res) {
      if (!err && res.statusCode == 200) {
        ...
        connection.query(
          `INSERT INTO tb_endereco (id_cadastro, rua, numero, complemento, bairro, cidade, lat, lng, endereco_imagem)
            VALUES (${id_coleta},"${endereco.rua}",${endereco.numero},${endereco.complemento},"${endereco.bairro}","${endereco.cidade}", ${result_json.lat}, ${result_json.lng}, ${endereco_imagem});`,
          function (err, result) {
            if (err) throw err;
  ...
  function inserir_terreno(id_coleta, terreno) {
    connection.query(
      `INSERT INTO tb_lote (id_cadastro, id_ocupacao, id_patrimonio, id_relevo,	id_limTestada,	id_uso, id_caracterizacao, id_tipo) 
        VALUES (${id_coleta}, ${terreno.ocupacao},${terreno.patrimonio},${terreno.relevo},${terreno.limite_testada}, ${terreno.uso},${terreno.caracterizacao}, ${terreno.tipo});`,
      function (err, result) {
        if (err) throw err;
  ...
  function salvarImagem(base64str, urlImagem){
    let buffer = Buffer.from(base64str.toString(),"base64");
    let bitmap = buffer.toString("base64");

    let random = parseInt(Math.random()*10000);
    let fileName = `${Date.now()}-${random}.jpg`;

    fs.writeFileSync('./uploads/'+fileName,bitmap, 'base64', (err) =>{
      if(err) console.log('Conversão com erro!');
    });
